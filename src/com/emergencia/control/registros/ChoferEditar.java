...
public class ChoferEditar {
	@Wire private Window winChoferEditar;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		usuario = (Usuario) Executions.getCurrent().getArg().get("Chofer");
		if (usuario == null) {
			usuario = new Usuario();
			usuario.setEstado("A");
			persona = new Persona();
		...
		Messagebox.show("Desea guardar el registro?", "Confirmación de Guardar", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION, new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (event.getName().equals("onYes")) {		
					try {						
						usuario.setPerfil(perfilDAO.getPerfilPorId(Globales.codigoChofer));
						...
						if (usuario.getIdUsuario() == null) {
							...
							persona.setCargo("Chofer");
							persona.setReferenciaDomiciliaria(persona.getDireccionDomiciliaria());
							...
							usuario.getPersona().setReferenciaDomiciliaria(persona.getDireccionDomiciliaria());
